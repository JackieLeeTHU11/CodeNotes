...
#include "caffe/layers/softmax_loss_layer_classweight_fiveclass.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void SoftmaxWithLossClassweightFiveclassLayer<Dtype>::LayerSetUp(
  ...
  if (1) {
    ...
    vector<int> count_shape(1, classweightsize);
     ...
     */
    CHECK_EQ(classweightsize, bottom[0]->channels())
    		<< "Number of class weight values does not match the number of classes.";

    float* label_count_data = label_counts_.mutable_cpu_data();
    
    vector<float> classweight;
    classweight.push_back(1.0);
    classweight.push_back(1.0);
    classweight.push_back(5.0);
    classweight.push_back(5.0);
    classweight.push_back(5.0);
    ...
    for (int i = 0; i < this->layer_param_.loss_param().class_weighting_size(); i++) {
        ...
        label_count_data[i] = classweight[i];
    }
...
template <typename Dtype>
void SoftmaxWithLossClassweightFiveclassLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SoftmaxWithLossClassweightFiveclassLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SoftmaxWithLossClassweightFiveclassLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(SoftmaxWithLossClassweightFiveclassLayer);
#endif

INSTANTIATE_CLASS(SoftmaxWithLossClassweightFiveclassLayer);
REGISTER_LAYER_CLASS(SoftmaxWithLossClassweightFiveclass);
